...
MS_cpgs <- read.xlsx("Data/401_2023_2596_MOESM3_ESM.xlsx", startRow = 2, colNames = T)
# custom background from which significant CpGs were found
background <- read.csv("Data/CpGs_NvsL.csv",header = T, row.names = 1)
...
ggsave("Figures/Upsets_MS.svg", plot = combined, device = "svg", width = 160, height = 160, units = "mm")
ggsave("Figures/S10.svg", plot = combined, device = "svg", width = 160, height = 160, units = "mm")
...
MS_background <- background$IlmnID
levels <- c(1,2,3)
...
for (level in levels){
  ...
  perm_res <- future_map(1:n_perm, function(x) {
    rand_cpgs <- matched_sample(input_cpgs = MS_cpgs$CpG.probe, background = background, manifest = manifest_selected)
  ...
  filename <- paste("Figures/OR_perm_MS_", level ,".RDS", sep = "")
...
# loading all plots
plots1 <- readRDS("Figures/OR_perm_MS_1.rds")
plots2 <- readRDS("Figures/OR_perm_MS_2.rds")
plots3 <- readRDS("Figures/OR_perm_MS_3.rds")
...
ggsave("Figures/S11.svg", plot = combined, device = "svg", width = 210, height = 160, units = "mm")