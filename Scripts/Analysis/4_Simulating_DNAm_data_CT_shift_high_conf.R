...
for (noise in noise_seq){
  ...
  for (i in 1:n_replicates){
    ...
    NeuN_set_CpGs[NeuN_seq,] <- replicate(sample_num, rtruncnorm(NeuN_set_size, a = 0, b = 1, mean = cpg_means[NeuN_seq], sd = sd_hetero))
    ...
    IRF8_set_CpGs[IRF8_seq,] <- replicate(sample_num, rtruncnorm(IRF8_set_size, a = 0, b = 1, mean = cpg_means[IRF8_seq], sd = sd_hetero))
    ...
    SOX10_set_CpGs[SOX10_seq, ] <- replicate(sample_num, rtruncnorm(SOX10_set_size, a = 0, b = 1, mean = cpg_means[SOX10_seq], sd = sd_hetero))
    ...
    TN_set_CpGs[TN_seq, ] <- replicate(sample_num, rtruncnorm(TN_set_size, a = 0, b = 1, mean = cpg_means[TN_seq], sd = sd_hetero))
    ...
    if (noise != 0){ # if statement is important due to the seed being set at the start (noise of 0 does not "consume" random number string)
      # generate shifted cell type proportions
    ...
    sim_result[counter, "overlap"] <- sum(!grepl("non", sig_CpGs[6,]))
    sim_result[counter, "non_overlap"] <- sum(grepl("non", sig_CpGs[6,]))
    
    sim_result_FDR[counter, "overlap"] <- sum(!grepl("non", sig_CpGs_FDR[6,]))
    sim_result_FDR[counter, "non_overlap"] <- sum(grepl("non", sig_CpGs_FDR[6,]))
    ...
    ORA_res[counter,"p_val"] <- phyper(q = NeuN_overlap - 1 ,
                                       m = length(sig_CpGs)/6,
                                       n = nrow(bulk_data) - length(sig_CpGs)/6,
                                       k = NeuN_set_size, lower.tail = FALSE)
    
    ORA_res[counter,"p_FDR"] <- phyper(q = NeuN_overlap - 1 ,
                                       m = length(sig_CpGs_FDR)/6,
                                       n = nrow(bulk_data) - length(sig_CpGs_FDR)/6,
                                       k = NeuN_set_size, lower.tail = FALSE)
    if (NeuN_overlap > 0) {
...
stopCluster(cl)
# save the data
#save.image("sim_workspaces/DNAm_sim_CT_lvl1.RData")

# load the results
load("sim_res_shift_high.RData")
...
plotting_NEU$RMSE <- rep(colMeans(rmse_mat[,2:17]), 2)
...
plotting_MG$RMSE <- rep(colMeans(rmse_mat[,2:17]), 2)
...
plotting_OLIG$RMSE <- rep(colMeans(rmse_mat[,2:17]), 2)
...
plotting_AST$RMSE <- rep(colMeans(rmse_mat[,2:17]), 2)
...
# save the figure
ggsave("Figures/CT_sim_lvl1.svg", device = "svg", width = 160, height = 90, units = "mm")
